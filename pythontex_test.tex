...
\subsubsection*{Model development steps}

\subsubsection*{Models implemented}

The decision for choosing each model was based on the hability to solve classification problems. Those are the models applied to this project:
1.	Logistic Regression with L1 regularization
2.	Random Forest (baseline)
3.	Random Forest (Optimized with hyperparameters)
4.	XGBoost (baseline)
5.	XGBoost (Optimized with hyperparameters)
6.	SVM 
7.	Decision Tree
...
\textbf{Logistic Regression with L1 regularization:} "ss" has the highest positive coefficient, while "age" has a significant negative coefficient, indicating their contrasting roles in prediction. Here, Lasso excluded one predictor:  impulsiveness.

\textbf{Linear SVM:} The feature "ss" is the most significant positive predictor, while "age" contributes negatively with moderate influence, followed by smaller coefficients for other features.

\textbf{Bagging Classifier:} "Age" and "ss" are the most influential features, with "age" showing the strongest predictive importance.

\textbf{Decision Tree:} "Age" dominates the predictive importance, followed by "ss" and other features with relatively smaller contributions.

\textbf{Random Forest:} "ss" and "age" are the most influential features, with other features showing moderate importance.

\textbf{Optimized XGBoost:} "Age" is the most significant feature, followed by "ss," with other features having a minor impact.

\textbf{Optimized Random Forest:} Both "ss" and "age" contribute highly to prediction, with balanced importance among other features. 

\textbf{Conclusion:} The features "ss" (sensation seeking) and "age" consistently emerge as the most important predictors across most models.
...
print(r"\small")
print(r"\begin{center}")
# Table 3: Model Performance using PythonTeX print() format
